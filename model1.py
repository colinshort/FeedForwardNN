...
def backpropagation(input, y, layer1, layer2, output, w1, w2, w3, z1, z2, z3, m):
    ...
    layer2_error = np.dot(w3.T, output_error) * sigmoid_prime(z2)
    layer1_error = np.dot(w2.T, layer2_error) * sigmoid_prime(z1)
...
def test_single_prediction(index, x_train,y_train, w1, w2, w3, b1, b2, b3):
    current_image = x_train[:, index, None]
    prediction = make_predictions(x_train[:, index, None], w1, w2, w3, b1, b2, b3)
    label = y_train[index]
...
x_train = x_train.reshape(x_train.shape[0], WIDTH * HEIGHT).T # flatten training inputs into 6000 784x1 column vectors
print(x_test.shape)
WIDTH = x_test.shape[1]
HEIGHT = x_test.shape[2]
x_test = x_test.reshape(x_test.shape[0], WIDTH * HEIGHT).T #flatten testing inputs into 600 784x1 column vectors
